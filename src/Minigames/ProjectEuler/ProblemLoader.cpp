...
Problem::Ptr ProblemLoader::load(unsigned int problemNumber)
{
   Problem::Ptr ret;

   if (problemNumber <= _problems.size())
   {
      ret = _problems.at(problemNumber);
   }
   else
   {
      ret = Problem::Ptr(new UnsolvedProblem);
   }

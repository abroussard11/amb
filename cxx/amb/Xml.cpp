...
Xml::Xml(const std::string& fileName) { loadFile(fileName); }
...
String Xml::getElement(std::string tagName) { return node.getElement(tagName); }
...
void Xml::parseData(const std::string& xmlStr) { node = xml::Node(xmlStr); }

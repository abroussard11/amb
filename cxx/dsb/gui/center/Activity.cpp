...
Activity::Activity(amb::xml::Node& item)
  ...
  auto bounds = rectUnion(nameBounds,
                          rectUnion(descBounds, rectUnion(LUBounds, UCBounds)));
  auto buf = 2.F;
  _boundingBox.setSize(
      sf::Vector2f(bounds.width + (2 * buf), bounds.height + (3 * buf)));
...
void Activity::onMouseButtonReleased(sf::Event& event) {
  ...
  if (isClickOnSelf(event) && event.mouseButton.button == sf::Mouse::Left) {
    color = sf::Color::Red;
    std::cout << "Sending ActivityMsg: " << _name.getString().toAnsiString()
              << std::endl;
    _msg.send();
  }
...
void Activity::updateSelf() {
  auto box = rectUnion(
      _name.getGlobalBounds(),
      rectUnion(_desc.getGlobalBounds(),
                rectUnion(_LU.getGlobalBounds(), _UC.getGlobalBounds())));
  _boundingBox.setSize(sf::Vector2f(box.width, box.height));
  _boundingBox.setPosition(box.left, box.top);
}

void Activity::drawSelf(sf::RenderTarget& target,
